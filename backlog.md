...
| **T6** | **Backup de la BD** — 🟡 script+cron corregidos y funcionando desde el 2026-08-19 (llevaban desde el 29 de mayo fallando en silencio por un `mkdir -p` faltante — ver `deploy.md` §7); **falta el restore de prueba verificado** | Ya se desplegaron **cuatro migraciones** sobre datos reales; el backup automático recién empezó a funcionar de verdad hoy — nunca se probó un restore |
...
| ~~ISS-048~~ | ~~Sin forma de volver desde "¿Cómo vas a pagar?" en `menu.html` a la carta~~ | ✅ **Hecho 2026-08-19**, pendiente de deploy. Encontrado por el usuario probando, no por la dueña del piloto — ver `ISS-048` |
