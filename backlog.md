...
| **PENSIONISTAS** | **Frontend del módulo** — el backend ya está entero y montado, con tests; falta el panel del owner, `pensionista.html` y el módulo JS. Además `login.html:420` no tiene `pensionista` en `ROLE_REDIRECT` (hoy entra a `undefined`) | 🟠 Prioridad del usuario, y es lo que más descomprime el atraso: no hay que decidir nada de negocio, solo construir pantallas sobre una API que ya funciona |
