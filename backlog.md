...
| ~~ISS-060~~ | ~~Pensionistas sin camino claro a `pensionista.html`~~ | ✅ **Hecho 2026-09-07, desplegado 2026-09-08** (`fdc9877`, opción C) — link "🧾 ¿Eres pensionista?" en el header de `menu.html` → `/login` + botón de instalar PWA en `pensionista.html`. Frontend puro, 13/13 E2E nuevo + sin regresión. Falta verlo usado por un pensionista real — ver `ISS-060` |
...
| ~~ISS-080~~ | ~~Stock agotado a mitad de un pedido con varios menús obliga a rehacer todo desde cero; no se puede editar un menú puntual en el carrito~~ | ✅ **Hecho 2026-08-27, desplegado 2026-08-28.** Rediseño completo del flujo de Pedir: cantidad primero (stepper), configurar después (picker encadenado), "✏️ Editar" por unidad. Prototipo interactivo aprobado antes de codear. 24/24 E2E nuevo (`test-pedir-cantidad-primero.js`) + 469/469 jest. **Validado en servicio real:** la dueña lo ha visto con sus comensales todo el piloto; la encuesta de ISS-081 (24 comensales reales) dio 95% positivo y 94% por el flujo nuevo. |
| ~~ISS-081~~ | ~~Pago en 1 sola pantalla (sacar "Revisa tu pedido") + aviso temporal + encuesta de producto~~ | ✅ **Hecho 2026-08-28, desplegado.** `enviarPago()` absorbe el repaso sin perder la garantía de Gap 17. Banner + encuesta (3 días), respuestas solo en `menupro.tech/admin`. **Encuesta cerrada 2026-08-30: 24 comensales reales del piloto 1 — 95% valoración positiva, 94% prefiere el flujo nuevo.** ISS-080 + ISS-081 validados del lado del comensal. 16/16 E2E (`test-feedback-flujo.js`) + 469/469 jest. Ver `ISS-081` §Resultados |
...
✅ **Implementado 2026-09-07, desplegado 2026-09-08** (`fdc9877`) — ver `ISS-060`. Falta uso real.
