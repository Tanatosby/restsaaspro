...
| Crear mesas de una vez (ISS-094) | 2026-09-13 | Configuración → Mesas: *"¿Cuántas mesas tiene tu local? [N] → Crear mesas"* con `POST /api/mesas/lote` (`utils/mesas.js`, crea las 1..N que falten, nunca borra) + resumen por rangos; se retiró el alta de a una con capacidad y la ✕ por mesa. QR por mesa arranca con esa cantidad. "Agregar manual" escribe la mesa (input numérico) en vez de elegirla de la tabla — la tabla `mesas` hoy solo alimenta el Plano. Tests: `mesas-lote.test.js` 31 + `scripts/test-iss094-mesas.js` 30. Sin desplegar. |
